...
// UpdateByID update a users by id, support partial update
...
func (d *usersDao) updateDataByID(ctx context.Context, db *gorm.DB, table *model.Users) error {
	...
	if table.UserGender != "" {
		update["user_gender"] = table.UserGender
	}
	if table.NickName != "" {
		update["nick_name"] = table.NickName
	}
	if table.UserPhone != "" {
		update["user_phone"] = table.UserPhone
	}
	if table.UserEmail != "" {
		update["user_email"] = table.UserEmail
	}
	if table.Status != "" {
