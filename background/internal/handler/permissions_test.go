...
func newPermissionsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewPermissionsDao(d.DB, c.ICache.(cache.PermissionsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &permissionsHandler{iDao: d.IDao.(dao.PermissionsDao)}
	iHandler := h.IHandler.(PermissionsHandler)
...
func Test_permissionsHandler_Create(t *testing.T) {
	h := newPermissionsHandler()
	defer h.Close()
	testData := &types.CreatePermissionsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Permissions))
...
func Test_permissionsHandler_DeleteByID(t *testing.T) {
	h := newPermissionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Permissions)
...
func Test_permissionsHandler_UpdateByID(t *testing.T) {
	h := newPermissionsHandler()
	defer h.Close()
	testData := &types.UpdatePermissionsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Permissions))
...
func Test_permissionsHandler_GetByID(t *testing.T) {
	h := newPermissionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Permissions)
...
func Test_permissionsHandler_List(t *testing.T) {
	h := newPermissionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Permissions)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListPermissionssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewPermissionsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewPermissionsHandler()
}
