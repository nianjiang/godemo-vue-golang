...
func newMenusHandler() *gotest.Handler {
	...
	d.IDao = dao.NewMenusDao(d.DB, c.ICache.(cache.MenusCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &menusHandler{iDao: d.IDao.(dao.MenusDao)}
	iHandler := h.IHandler.(MenusHandler)
...
func Test_menusHandler_Create(t *testing.T) {
	h := newMenusHandler()
	defer h.Close()
	testData := &types.CreateMenusRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Menus))
...
func Test_menusHandler_UpdateByID(t *testing.T) {
	h := newMenusHandler()
	defer h.Close()
	testData := &types.UpdateMenusByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Menus))
...
func Test_menusHandler_GetByID(t *testing.T) {
	h := newMenusHandler()
	defer h.Close()
	testData := h.TestData.(*model.Menus)
...
func Test_menusHandler_List(t *testing.T) {
	h := newMenusHandler()
	defer h.Close()
	testData := h.TestData.(*model.Menus)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListMenussRequest{query.Params{
		Page:  0,
		Limit: 10,
